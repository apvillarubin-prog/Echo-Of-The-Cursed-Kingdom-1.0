...
public:
	...
	void actually_teleport() {
		...
		if (self) {
			...
			TypedArray<Node> spikes = self->get_tree()->get_nodes_in_group("spikes");
			for (int i = 0; i < spikes.size(); i++) {
				Node* spike = Object::cast_to<Node>(spikes[i]);
				if (spike) spike->set_meta("already_triggered", false);
			}
	...
	void on_awake(CharacterBody2D* node) {
		self = node;
	...
	void on_physics_process(double delta) {
		...
		if (priest_action_timer > 0.0f) {
			...
			if (priest_action_timer <= 0.0f) { is_healing = false; is_buffing = false; }
		}
		if (wizard_action_timer > 0.0f) {
			wizard_action_timer -= (float)delta;
			if (wizard_action_timer <= 0.0f) { is_summoning = false; is_teleporting = false; }
		}
		...
		if (current_hero == KNIGHT && has_shield) {
			...
			if (key_q_just_pressed && !is_blocking && block_cooldown_val <= 0.0f && attack_timer <= 0.0f && self->is_on_floor()) {
				is_blocking = true; block_timer = BLOCK_DURATION;
				if (sfx_block) sfx_block->play();
				is_action_locked = true;
			}
			if (is_blocking) {
				block_timer -= (float)delta;
				if (block_timer <= 0.0f) { is_blocking = false; block_cooldown_val = 1.0f; }
			}
		} else {
			is_blocking = false;
		}
		...
		if (current_hero == PRIEST && !is_action_locked && self->is_on_floor()) {
			if (key_e_just_pressed && has_heal && priest_heal_cooldown_val <= 0.0f) {
				is_healing = true; priest_action_timer = 1.2f; priest_heal_cooldown_val = 3.0f;
				if (sfx_priest_heal) sfx_priest_heal->play();
				int old_hp = player_health; player_health += 10;
				...
				is_buffing = true; priest_action_timer = 1.2f; priest_buff_cooldown_val = 3.0f; buff_active_timer = 10.0f;
		...
		if (current_hero == WIZARD && !is_action_locked && self->is_on_floor()) {
			if (mouse_left_just_pressed && has_summon && wizard_summon_cooldown_val <= 0.0f) {
				is_summoning = true; wizard_action_timer = 1.6f; wizard_summon_cooldown_val = 3.0f; is_action_locked = true;
				if (sfx_wizard_summon) sfx_wizard_summon->play();
				if (sprite) { sprite->stop(); last_attack_frame = -1; }
				...
				is_teleporting = true; wizard_action_timer = 1.4f; wizard_teleport_cooldown_val = 2.0f; is_action_locked = true;
		...
		if (attack_timer > 0.0f) {
			...
			if (current_hero == KNIGHT && has_sword) {
				is_attacking = true; attack_timer = knight_attack_duration; is_action_locked = true;
				if (sfx_attack) sfx_attack->play();
				if (sprite) { sprite->stop(); last_attack_frame = -1; }
			} else if (current_hero == ARCHER && has_bow) {
				is_attacking = true; attack_timer = archer_attack_duration; is_action_locked = true;
		...
		if (input->is_action_just_pressed("ui_accept") && self->is_on_floor()) {
			velocity.y = jump_velocity; 
			if (is_action_locked) {
				attack_timer = 0.0f; self->set_meta("attack_timer", 0.0f);
				is_attacking = false; is_blocking = false; is_healing = false;
				is_buffing = false; is_summoning = false; is_teleporting = false;
				block_timer = 0.0f; priest_action_timer = 0.0f; wizard_action_timer = 0.0f;
		...
		if (self->is_on_floor() && velocity.x != 0 && !is_action_locked) {
			if (sfx_walk && !sfx_walk->is_playing()) sfx_walk->play(); 
		} else {
			if (sfx_walk && sfx_walk->is_playing()) sfx_walk->stop(); 
		}
			...
			for (int i = 0; i < targets.size(); i++) {
				...
				if (hook) {
					...
					if (current_distance < shortest_distance) {
						shortest_distance = current_distance; nearest_valid_hook = hook;
					}
				}
			}
			...
			if (is_blocking) {
				...
				if (current_frame >= damage_frame && last_attack_frame < damage_frame) {
					...
					if (current_hero == KNIGHT) {
						...
						if (!hit_anything) UtilityFunctions::print("[DEBUG-PLAYER] Knight swung but missed.");
					} 
		...
		if (!was_on_floor && is_currently_on_floor) {
			if (sfx_landing) sfx_landing->play();
		}
...
// ============================================================================
// 2. INSTANCE MAPPER & JENOVA BINDINGS
// ============================================================================
...
void unlock_sword(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_sword = true; 
	Engine::get_singleton()->set_meta("save_has_sword", true); 
}
void unlock_shield(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_shield = true; 
	Engine::get_singleton()->set_meta("save_has_shield", true); 
}
void unlock_bow(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_bow = true; 
	Engine::get_singleton()->set_meta("save_has_bow", true); 
}
void unlock_grapple(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_grapple = true; 
	Engine::get_singleton()->set_meta("save_has_grapple", true); 
}
void unlock_heal(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_heal = true; 
	Engine::get_singleton()->set_meta("save_has_heal", true); 
}
void unlock_buff(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_buff = true; 
	Engine::get_singleton()->set_meta("save_has_buff", true); 
}
void unlock_summon(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_summon = true; 
	Engine::get_singleton()->set_meta("save_has_summon", true); 
}
void unlock_teleport(Caller* instance) { 
	if (auto p = get_player(instance)) p->has_teleport = true; 
	Engine::get_singleton()->set_meta("save_has_teleport", true); 
}

int increase_inventory(Caller* instance) { 
	if (auto p = get_player(instance)) {
		p->inventory_count += 1; 
...
int get_inventory_count(Caller* instance) { 
	if (auto p = get_player(instance)) return p->inventory_count; 
	return 0; 
}

void actually_teleport(Caller* instance) {
	if (auto p = get_player(instance)) p->actually_teleport();
}

void respawn() {
	if (fallback_player) fallback_player->respawn();
}

bool take_damage(int amount) {
	if (fallback_player) return fallback_player->take_damage(amount);
	return false; 
}

void OnAwake(Caller* instance) {
	if (auto p = get_player(instance)) {
		p->on_awake(GetSelf<CharacterBody2D>(instance));
	}
}
